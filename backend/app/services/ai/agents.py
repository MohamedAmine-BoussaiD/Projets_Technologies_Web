...
def generate_exercise(
    ...
    context_text = (
        "\n\n".join(context)
        if context
        else ("Pas de support spécifique fourni. Utilise tes connaissances générales.")
    )
...
def _mcq_check_node(state: _CorrectionState) -> _CorrectionState:
    ...
    if is_single_letter:
        correct_letter = (
            expected.strip().upper()[:1] if len(expected.strip()) == 1 else ""
        )
...
def _update_level_node(state: _CorrectionState) -> _CorrectionState:
    ...
    new_level = round(
        max(ai_settings.level_min, min(ai_settings.level_max, new_level)), 3
    )
